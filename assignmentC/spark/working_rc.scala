...
def reorg(datadir :String) {
  ...
  val person_final = person_filtered_1.select($"personId", $"birthday").distinct

  // preparing to new files to write
  val knows_final = knows_filtered_2.withColumnRenamed("tempPersonId", "personId")

  person_final.write.format("parquet").mode("overwrite").save(datadir + "/person.parquet")
...
def cruncher(datadir :String, a1 :Int, a2 :Int, a3 :Int, a4 :Int, lo :Int, hi :Int) :org.apache.spark.sql.DataFrame =
{
  ...
  val interest = spark.read.parquet(datadir + "/interest.parquet/*").cache()
  val focus = interest.filter($"interest" isin (a1, a2, a3, a4)).withColumn("nofan", $"interest".notEqual(a1))
  ...
  val scores = person.join(focus, "personId").groupBy("birthday", "personId").agg(count("personId") as "score", min("nofan") as "nofan")
